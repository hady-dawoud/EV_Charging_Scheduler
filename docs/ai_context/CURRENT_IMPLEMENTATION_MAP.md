...
- `apps/api/app/main.py`: creates the FastAPI app, configures CORS, includes system, station, and recommendation routers.
...
- `apps/mobile/src/services/api.ts`: builds the backend JSON payload and calls `POST /recommendations`.
