...
- The app/runtime path can now run the final feeder MaskablePPO checkpoint in strict offline verification when the local feeder package and optional Torch/SB3/sb3-contrib dependencies are installed.
- Strict no-fallback verification is `scripts/verification/verify_feeder_rl_api_no_fallback.py --strict`.
- The checkpoint consumes the existing 2200-feature observation (`10 + 73 * 30`) and does not consume forecasting features.
- This remains single-agent offline feeder station/node selection over recorded replay, not live DigitalTwin closed-loop control or MARL.
